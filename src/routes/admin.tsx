...
function Dashboard() {
  ...
  function exportCsv(rows: Rsvp[]) {
    const headers = ["Nome", "Confirmou", "Acompanhantes", "Telefone", "Restricao", "Mensagem", "Data"];
    const csv = [
      headers.join(","),
      ...rows.map((r) =>
        [
      ...
      <header className="border-b border-border bg-foreground text-background">
        <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Link to="/" className="px-4 py-2 text-xs uppercase tracking-[0.25em] border border-background/30 hover:border-gold transition-colors">Site</Link>
            <button onClick={() => supabase.auth.signOut()} className="px-4 py-2 text-xs uppercase tracking-[0.25em] bg-gold text-foreground hover:bg-white transition-colors">Sair</button>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="flex flex-wrap gap-1 border-b border-border mb-6">
          {(["overview", "rsvps", "gifts"] as const).map((k) => (
            <button key={k} onClick={() => setTab(k)} className={`px-5 py-3 text-[10px] sm:text-xs uppercase tracking-[0.3em] border-b-2 transition-all ${tab === k ? "border-gold text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
              {k === "overview" ? "Mensagens" : k === "rsvps" ? `Convidados (${rsvps.length})` : `Presentes (${gifts.length})`}
            </button>
          ))}
        </div>
        ...
        {tab === "overview" && (
          <div className="grid gap-4 md:grid-cols-2">
            {rsvps.filter((r) => r.message).map((r) => (
              <div key={r.id} className="p-6 border border-border bg-card rounded-md shadow-sm relative overflow-hidden group">
                <div className="absolute top-0 left-0 w-1 h-full bg-gold opacity-50 group-hover:opacity-100 transition-opacity" />
                <p className="text-foreground/90 italic font-light leading-relaxed">"{r.message}"</p>
                <div className="mt-4 flex items-center justify-between">
                  <p className="text-[10px] uppercase tracking-[0.3em] text-gold">— {r.full_name}</p>
                  <span className="text-[9px] text-muted-foreground">{new Date(r.created_at).toLocaleDateString("pt-BR")}</span>
                </div>
              </div>
            ))}
            {rsvps.filter((r) => r.message).length === 0 && <p className="text-muted-foreground py-10 text-center col-span-2">Nenhuma mensagem ainda.</p>}
          </div>
        )}
        ...
        {tab === "rsvps" && (
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <button onClick={() => exportCsv(filteredRsvps)} className="px-6 py-3 bg-foreground text-background text-[10px] uppercase tracking-[0.3em] hover:bg-gold hover:text-foreground transition-all shadow-md">
                Exportar CSV
              </button>
            </div>
            ...
            <div className="overflow-x-auto border border-border rounded-md bg-card">
              <table className="w-full text-sm">
                <thead className="bg-secondary/50 text-[10px] uppercase tracking-[0.2em] border-b border-border">
                  <tr>
                    <Th>Nome</Th><Th>Status</Th><Th>Acomp.</Th><Th>Telefone</Th><Th>Restrição</Th><Th className="hidden sm:table-cell">Data</Th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border/50">
                  {filteredRsvps.map((r) => (
                    <tr key={r.id} className="hover:bg-secondary/20 transition-colors">
                      ...
                      <Td>
                        <span className={`text-[10px] uppercase tracking-widest px-2 py-1 rounded-full ${r.attending ? "bg-gold/10 text-gold" : "bg-muted/10 text-muted-foreground"}`}>
                          {r.attending ? "Sim" : "Não"}
                        </span>
                      </Td>
                      <Td className="text-center">{r.companions}</Td>
                      <Td className="text-xs text-muted-foreground">{r.phone || "—"}</Td>
                      <Td className="max-w-[150px] truncate text-xs" title={r.dietary_restrictions || ""}>{r.dietary_restrictions || "—"}</Td>
                      <Td className="text-xs text-muted-foreground hidden sm:table-cell">{new Date(r.created_at).toLocaleDateString("pt-BR")}</Td>
                    </tr>
                  ))}
                  {filteredRsvps.length === 0 && (<tr><td colSpan={6} className="p-12 text-center text-muted-foreground">Nenhum convidado encontrado.</td></tr>)}
                </tbody>
              </table>
            </div>
          </div>
        ...
        {tab === "gifts" && (
          <div className="space-y-6">
            <div className="flex justify-between items-end p-6 bg-card border border-border rounded-md shadow-sm">
              <div>
                <p className="text-[10px] uppercase tracking-[0.4em] text-muted-foreground">Total arrecadado</p>
                <p className="font-display text-5xl text-gold mt-1">R$ {giftTotal.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</p>
              </div>
              <div className="text-right">
                <p className="text-[10px] uppercase tracking-[0.4em] text-muted-foreground">Contribuições</p>
            ...
            <div className="overflow-x-auto border border-border rounded-md bg-card">
              <table className="w-full text-sm">
                <thead className="bg-secondary/50 text-[10px] uppercase tracking-[0.2em] border-b border-border">
                  <tr><Th>Convidado</Th><Th>Cota</Th><Th>Valor</Th><Th className="hidden sm:table-cell">Data</Th></tr>
                </thead>
                <tbody className="divide-y divide-border/50">
                  {gifts.map((g) => (
                    <tr key={g.id} className="hover:bg-secondary/20 transition-colors">
                      ...
                      <Td className="text-gold font-medium">R$ {Number(g.amount).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</Td>
                      <Td className="text-xs text-muted-foreground hidden sm:table-cell">{new Date(g.created_at).toLocaleDateString("pt-BR")}</Td>
                    </tr>
                  ))}
                  {gifts.length === 0 && (<tr><td colSpan={4} className="p-12 text-center text-muted-foreground">Nenhuma contribuição ainda.</td></tr>)}
                </tbody>
              </table>
            </div>
          </div>
...
function Stat({ label, value, accent }: { label: string; value: number; accent?: boolean }) {
  return (
    <div className={`p-6 rounded-md border transition-all hover:shadow-md ${accent ? "bg-foreground text-background border-foreground" : "bg-card border-border"}`}>
      <p className={`text-[10px] uppercase tracking-[0.3em] ${accent ? "text-gold" : "text-muted-foreground"}`}>{label}</p>
      <p className="font-display text-4xl mt-2">{value}</p>
    </div>
  );
}

const Th = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => <th className={`text-left px-6 py-4 font-medium ${className}`}>{children}</th>;
const Td = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => <td className={`px-6 py-4 ${className}`}>{children}</td>;
