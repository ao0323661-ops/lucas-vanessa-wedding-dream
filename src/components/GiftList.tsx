      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {WEDDING.gifts.map((g, i) => (
          <motion.button
            ...
            whileHover={{ y: -8 }}
            whileTap={{ scale: 0.99 }}
            className="editorial-panel group relative overflow-hidden rounded-md p-7 text-left transition-all duration-500 hover:border-olive/45 sm:p-8"
          >
            <div className="pointer-events-none absolute inset-x-6 top-0 h-px bg-gradient-to-r from-transparent via-sage/65 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
            <div className="editorial-kicker mb-6">Cota {String(i + 1).padStart(2, "0")}</div>
            ...
            <div className="mt-8 flex items-end justify-between gap-4">
              ...
              <span className="flex h-11 w-11 items-center justify-center rounded-full border border-olive/25 text-olive transition-all duration-500 group-hover:border-olive group-hover:bg-olive group-hover:text-background">
                <ArrowRight size={16} />
              </span>
            </div>
          </motion.button>
      ...
      {selected && (
        ...
        >
          ...
          >
            <button
              onClick={() => setSelected(null)}
              className="absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-full border border-border bg-background/70 text-muted-foreground transition-colors hover:border-olive hover:text-olive"
            ...
            <div className="editorial-divider editorial-kicker">Presente</div>
            ...
            {!done ? (
              <div className="mt-7 space-y-5">
                ...
                <div className="rounded-md border border-olive/20 bg-olive/[0.06] p-4 text-sm">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <button
                      onClick={copyPix}
                      className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-olive text-background transition hover:bg-olive-deep"
                ...
                <div className="flex flex-col gap-3 pt-2 sm:flex-row">
                  <button
                    onClick={() => setSelected(null)}
                    className="min-h-[48px] flex-1 rounded-md border border-border text-xs uppercase tracking-[0.28em] transition-colors hover:border-olive hover:text-olive"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="shine-line flex min-h-[48px] flex-1 items-center justify-center gap-3 rounded-md bg-olive px-4 text-xs uppercase tracking-[0.28em] text-background transition hover:bg-olive-deep disabled:cursor-wait disabled:opacity-55"
                  >
              ...
              <div className="mt-7 py-8 text-center">
                <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full border border-olive/30 bg-olive/[0.08] text-olive">
                  <Check size={22} />
                </div>
                ...
                <button
                  onClick={() => setSelected(null)}
                  className="shine-line mt-7 inline-flex min-h-[48px] items-center justify-center rounded-md bg-olive px-8 text-xs uppercase tracking-[0.28em] text-background transition hover:bg-olive-deep"
                >
                  Fechar
                </button>
              </div>
